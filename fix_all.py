...
# Let's insert the missing function block right after `/* ─── BUILDER LOGIC ─── */`
missing_functions = """
        var debounceTimer;
            ...
            lineItems.forEach(function(item, index) {
                ...
                if (bizType === 'product') {
                    ...
                    html += '<div class="field-row">'
                        ...
                        + '</div>'
                        + '<div class="field-row single"><div class="field"><label>Appointment Date</label><input type="date" class="item-appt"></div></div>';
                }
                html += '</div>';
            });
...
"""
text = text.replace('/* ─── BUILDER LOGIC ─── */', '/* ─── BUILDER LOGIC ─── */' + missing_functions)
